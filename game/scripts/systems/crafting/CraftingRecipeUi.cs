...
public partial class CraftingRecipeUi : Panel
{
	...
	public void UpdateRecipe(Inventory inventory)
	{
		...
		foreach (var requirement in CraftingRecipe.Requirements)
			...
			{
				Log.Debug("Required items in inventory: " + inventory.GetNumberOfItems(requirement.Item));
				Log.Debug("Required amount for recipe: " + requirement.Amount);
				canCraft = false;
			}

		Log.Debug("Can craft: " + canCraft);
